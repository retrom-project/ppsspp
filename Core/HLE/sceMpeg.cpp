...
void __MpegInit() {
	...
	__MpegBaseInit();
	// flash0's mpeg.prx is deliberately not loaded here, but from the sceUtility AV-module
	// notification: a game either ships its own sceMpeg_library on the disc (Death Jr., Pursuit
	// Force) or asks sceUtility for one (Thrillville), and both arrive in time on their own.
	// Loading it here would cost 33KB at the top of user memory for nothing, and stacks allocate
	// downward from there - enough to push Pursuit Force's main thread stack below the 256KB
	// block it needs, which fails the boot outright.
	isMpegInit = false;
